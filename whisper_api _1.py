...
def _build_pool():
    ...
    Called once at module import time — works under both Gunicorn and __main__.
    """
...
# ── Pre-load at import time so Gunicorn workers have the model ready ───────────
# Gunicorn imports this module after fork — each worker gets its own pool copy.
_build_pool()
...
def transcribe():
    ...
    try:
        ...
        try:
            ...
            # IMPORTANT: segments is a lazy generator — must be fully consumed
            # here while the model is still checked out, BEFORE return_model().
            texts = [(seg.text or "").strip() for seg in segments]
        finally:
            return_model(model)   # safe — generator fully consumed above
...
if __name__ == "__main__":
    ...
    # Pool already loaded at module import above — just start the server.
